...
// npx cypress run --spec "cypress/e2e/TransactionChecker/Withdrawal/INSTAPAY_WITHDRAWAL.cy.js"
// npx cypress open
...
describe('Looping within an it block', () => {
    PageNav.forEach((pageNav) => {
        it(`Should test transactions for Page: ${pageNav}`, () => {
            ...
            // Filter transactions
            filterTransactions('getTransactionTypeWithdrawal', 'getTransactionAllBankVendor', 'getTransactionInstapaySolution', 1, pageNav, { timeout: 5500 });
...
const validateWebhookResponses = (transactionNumber, filpath, sheetName, sheetRow) => {
    ...
    try{
        ...
        readFilePayload.getSolutionRef().then((payload_solrefno) => {
            transactiondetails.getSolutionRef().invoke('text').then((solutionRef)=>{
                expect(payload_solrefno).to.eq(solutionRef);
            });
        });
        ...
        readFilePayload.getStatus().should((payload_status) => {
        ...
        readFileCallback.getCustomerMobile().then((callback_customer_mobile) => {
            transactiondetails.getMobile().invoke('text').then((customerMobile)=>{
